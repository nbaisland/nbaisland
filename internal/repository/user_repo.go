...
type UserRepository interface {
    ...
    UpdateCurrency(ctx context.Context, id int, currency string) error
...
func (r *PSQLUserRepo) GetByID(ctx context.Context, id int) (*models.User, error) {
	var u = &models.User{}
...
func (r *PSQLUserRepo) UpdateName(ctx context.Context, id int, name string) error {
	_, err := r.Pool.Exec(ctx, "UPDATE users SET name=$2 where id = $1", id, name)
	return err
}

func (r *PSQLUserRepo) UpdateEmail(ctx context.Context, id int, email string) error {
	_, err := r.Pool.Exec(ctx, "UPDATE users SET email=$2 where id = $1", id, email)
	return err
}

func (r *PSQLUserRepo) UpdatePassword(ctx context.Context, id int, password string) error {
	_, err := r.Pool.Exec(ctx, "UPDATE users SET password=$2 where id = $1", id, password)
	return err
}

func (r *PSQLUserRepo) UpdateCurrency(ctx context.Context, id int, currency string) error {
	_, err := r.Pool.Exec(ctx, "UPDATE users SET currency=$2 where id = $1", id, currency)
	return err
}

func (r *PSQLUserRepo) Delete(ctx context.Context, id int) error {
